...
#define TX_OPEN(d, t, f)						\
do {									\
	if (d->use_dtx) {						\
		rc = daos_tx_open(d->coh, &t, f, NULL);			\
		if (rc) {						\
...
#define TX_COMMIT(d, t)							\
do {									\
	if (d->use_dtx) {						\
		rc = daos_tx_commit(t, NULL);				\
		if (rc == -DER_TX_RESTART) {				\
			rc = daos_tx_restart(t, NULL);			\
			if (rc) {					\
...
#define TX_CLOSE(d, t)							\
do {									\
	if (daos_handle_is_valid(t)) {					\
		rc = daos_tx_close(t,  NULL);				\
		if (rc) {						\
